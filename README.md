...
#### 文档位置规范
- **Python项目文档**: 统一放在 `docs/` 目录
- **Web项目文档**: 统一放在 `web/docs/` 目录（如果有web/docs则放这里）或 `web/` 根目录
- **禁止**: 在项目根目录创建临时文档（除README.md外）

#### 文档提交规则 ⭐ 强制要求
1. **所有文档更新（无论大小）必须提交到Git**
   - 文档是项目的重要组成部分，不能只存在本地
   - 每次文档更新都应该有清晰的commit message
   - 示例提交：
     ```bash
     git add docs/
...
2. **文档命名规范**:
   - 使用大写+下划线：`FEATURE_NAME.md`
   - 或使用小写+连字符：`feature-name.md`
   - 保持命名一致性

3. **文档清理规则**:
   - 定期检查并删除过期文档
   - 临时文档完成后应移动到docs/目录或删除
   - 不要在根目录积累过多文档文件

#### Git忽略规则
- ✅ 提交：所有 `.md` 文档文件
- ❌ 不提交：`venv/`, `__pycache__/`, `*.pyc`, `node_modules/`
- ❌ 不提交：生成的图片 `*.png`, `*.jpg`（除非是文档插图）
...
- **v2.6.0** (2025-10-28) - Web国际化支持：3种语言（English/中文/日本語）零依赖i18n方案+智能语言检测 🌍
- **v2.5.0** (2025-10-28) - Web快速胜利更新：键盘快捷键+参数预设+迷宫难度评估+用户体验全面提升 ⚡
- **v2.4.0** (2025-10-28) - Web Island GA UI完整集成：并行计算完全可用+多岛屿可视化 🏝️
- **v2.3.0** (2025-10-28) - Web可视化与数据分析：3D景观+实时统计+数据导出系统 🎨
- **v2.2.0** (2025-10-28) - Web重大架构升级：多样化算子系统+混合GA+岛屿模型+增强自适应策略 🚀
